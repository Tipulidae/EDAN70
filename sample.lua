...
cmd:option('-primetext',"",'filepath, used as a prompt to "seed" the state of the LSTM using a given sequence, before we sample.')
...
local seed_file = opt.primetext

if string.len(opt.primetext) > 0 then
	f = assert(io.open(seed_file, 'r'))
	local seed_text = f:read("*all")
	f:close()
